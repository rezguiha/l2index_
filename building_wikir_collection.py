...
def main():
    ...
    print('------Creating Posting file, Vocabulary and Document length ', flush=True)
    Collection_wikir.build_inverted_index_and_vocabulary(args.indexed_path,save=True)

#     print('Compute fasstetext embeddings', flush=True)
#     Collection_wikir.compute_fasttext_embedding(args.fasttext_path)
    end=time.time()
    print('--------------------Finished------------------',flush=True)
    print("Total time = ", end-start, " s",flush=True)
