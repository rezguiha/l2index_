...
def main():
    ...
    #Initializing the results plot values for the different models
    ...
    #Loading validation and test query relavance values
    ...
    #Computing collection vocabulary size and total number of elements
    coll_vocab_size, coll_tot_nb_elem = utils.evaluate_inverted_index(Collection.inverted_index)
    #Evaluating the baseline models without TDV weights and saving the results of calidation and test partitions
    ...
    #Initialization of batch size, the loss function and optimizer for the training
    batch_gen_time = []
    batch_size = 64
    ...
    while epoch < args.nb_epoch and prop_elem_index > 0.2:
        ...
        begin = time.time()
        #Generating batches from WikIR Collection for training
        ...
        for i in range(len(query_batches)):
            with tf.GradientTape() as tape:
                ...
                #Computing the hinge loss , the regularization loss and total loss
        ...
        #Computing TDVs and saving them
        weights = model.compute_index()

        pickle.dump(weights, open('weights/' + args.experiment_name + '/epoch_' + str(epoch), 'wb'))
        #updating the inverted index and computing the new idf, doc lengths and collection frequencies
